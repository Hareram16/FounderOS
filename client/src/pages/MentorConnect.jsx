...
const MentorConnect = () => {
  ...
  const handleChat = async (mentorId) => {
    ...
    try {
      ...
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/mentors/${mentorId}/chat`,
        {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        "https://founderos-5t6s.onrender.com/api/messages",
        {
          method: "POST",
          headers: {
      ...
      if (res.ok) {
        ...
        const error = await res
          .json()
          .catch(() => ({ msg: "Failed to send message" }));
  ...
  const fetchMentors = async () => {
    try {
      const res = await fetch(
        "https://founderos-5t6s.onrender.com/api/mentors",
      );
      if (res.ok) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/mentors/${id}`,
        {
          method: "DELETE",
          headers: { "x-auth-token": token },
        },
  ...
  const handleMentorRequest = async (mentorId) => {
    try {
      ...
      const message =
        prompt("Add a message to your mentor request (optional):") || "";
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/mentors/${mentorId}/request`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (res.ok) {
        ...
        const error = await res
          .json()
          .catch(() => ({ msg: "Failed to send mentor request" }));
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      >
        ...
        >
          Get guidance from industry experts. Connect with mentors who can
            ...
            >
              ...
              <div style={{ display: "flex", gap: "1rem" }}>
                <img
                  src={
                    mentor.avatar ||
                    `https://api.dicebear.com/7.x/avataaars/svg?seed=${mentor.username}`
                  }
                  alt={mentor.username}
                  style={{
                    width: "80px",
            ...
            >
              ...
              >
                <img
                  src={
                    chatModal.avatar ||
                    `https://api.dicebear.com/7.x/avataaars/svg?seed=${chatModal.username}`
                  }
                ...
                <div>
                  ...
                  >
                    {chatModal.role}{" "}
                    {chatModal.company && `at ${chatModal.company}`}
                  </p>
                </div>
              </div>
              <button
