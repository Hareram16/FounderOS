  ...
  const fetchProjects = async () => {
    try {
      const res = await fetch(
        "https://founderos-5t6s.onrender.com/api/projects?search=Hiring",
      );
      if (res.ok) {
        const data = await res.json();
        // Filter projects that have "Hiring" tag and open roles
        const hiringProjects = data.filter(
          (p) =>
            p.tags &&
            p.tags.some((tag) => tag.toLowerCase() === "hiring") &&
            p.openRoles &&
            p.openRoles.length > 0 &&
            p.openRoles.some((r) => r.status === "open"),
        );
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      >
        ...
        >
          Find projects looking for team members. Browse open roles and connect
        ...
        >
          <p style={{ color: "var(--text-muted)", fontSize: "1.1rem" }}>
            {search
              ...
              : 'No projects with open roles at the moment. Add "Hiring" tag to your project to appear here!'}
          </p>
        </div>
      ) : (
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {
                      project.openRoles.filter((r) => r.status === "open")
                        .length
                    }{" "}
                    Open{" "}
                    {project.openRoles.filter((r) => r.status === "open")
