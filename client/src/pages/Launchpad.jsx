...
const Launchpad = () => {
  ...
  const fetchLaunches = async () => {
    try {
      const res = await fetch("https://founderos-5t6s.onrender.com/api/launch");
  ...
  const fetchMyProjects = async () => {
    ...
    try {
      ...
      const res = await fetch(
        "https://founderos-5t6s.onrender.com/api/projects/me",
        {
  ...
  const handleUpvote = async (launchId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/launch/${launchId}/upvote`,
        {
          method: "PUT",
          headers: { "x-auth-token": token },
        },
  ...
  const handleLaunchSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        "https://founderos-5t6s.onrender.com/api/launch",
        {
          method: "POST",
          headers: {
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: "center", marginBottom: "3rem" }}>
        ...
        >
          Discover the next big thing. Upvote, comment, and support fellow
          innovators.
        </p>
        <div
          style={{
        ...
        >
          ...
          >
            <Rocket size={20} style={{ marginRight: "0.5rem" }} /> Launch Your
            Project
          </button>
        </div>
      </div>
            ...
            >
              ...
              >
                <button
                  ...
                  style={{
                    background: launch.upvotes.includes(user?._id)
                      ? "var(--primary)"
                    ...
                    alignItems: "center",
                    gap: "0.5rem",
                    color: launch.upvotes.includes(user?._id)
                      ? "white"
                      : "var(--text-muted)",
                    cursor: "pointer",
                    transition: "all 0.2s",
              ...
              <div style={{ display: "flex", gap: "1rem" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setLaunchForm({ ...launchForm, tags: e.target.value })
                    }
                  />
                </div>
                <div style={{ flex: 1 }}>
