...
const ProjectWorkspace = () => {
  ...
  const fetchWorkspace = async () => {
    try {
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/projects/${id}/workspace`,
      );
      if (res.ok) {
  ...
  const addTask = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/projects/${id}/workspace/kanban`,
        {
          method: "PUT",
          headers: {
  ...
  const addNote = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/projects/${id}/workspace/notes`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const moveTask = async (taskId, targetColumn) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/projects/${id}/workspace/kanban/move`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    setWorkspace((prev) => {
      ...
      return {
        ...
        kanban: {
          ...prev.kanban,
          [activeContainer]: [
            ...prev.kanban[activeContainer].filter(
              (item) => item !== activeItems[activeIndex],
            ),
          ],
          [overContainer]: [
            ...prev.kanban[overContainer].slice(0, newIndex),
            activeItems[activeIndex],
            ...prev.kanban[overContainer].slice(
              newIndex,
              prev.kanban[overContainer].length,
            ),
          ],
        },
      };
          ...
          >
            {columns.map((column) => (
              ...
              >
                ...
                >
                  <h3 style={{ margin: 0, color: column.color }}>
                    {column.label}
                  </h3>
                  <span
                ...
                <SortableContext
                  id={column.key}
                  items={(workspace.kanban[column.key] || []).map(
                    (t) => t._id || t.id,
                  )}
