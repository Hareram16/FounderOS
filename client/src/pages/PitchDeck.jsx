  ...
  const generatePitchDeck = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        "https://founderos-5t6s.onrender.com/api/ai/pitch-deck",
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        try {
          ...
          if (errorJson.msg) {
            ...
            alert(
              `Failed to generate pitch deck: ${res.status} ${res.statusText}`,
            );
          }
        } catch {
          alert(
      ...
      try {
        ...
        alert(
          "Error parsing server response. The server may be experiencing issues. Please try again.",
        );
        return;
      }
      ...
      if (data && typeof data === "object") {
        ...
        if (data.error && !data.title) {
          alert(
            `Error: ${data.msg || data.error || "Failed to generate pitch deck"}`,
          );
          return;
        }
  ...
  return (
    <div style={{ padding: "2rem", maxWidth: "1200px", margin: "0 auto" }}>
      ...
      >
        ...
        >
          Generate investor-ready pitch deck content with one click. Export to
          PDF or slides.
        </p>
      </div>
