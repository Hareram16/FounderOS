...
const ProjectDetails = () => {
  ...
  const fetchContributions = async () => {
    try {
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/projects/${id}/contributions`,
      );
      if (res.ok) {
  ...
  const addOpenRole = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/projects/${id}/roles`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        const updatedProject = await fetch(
          `https://founderos-5t6s.onrender.com/api/projects/${id}`,
        ).then((r) => r.json());
  ...
  const handleLike = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/projects/like/${id}`,
        {
          method: "PUT",
  ...
  const handleComment = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `https://founderos-5t6s.onrender.com/api/projects/comment/${id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="glass-panel" style={{ padding: "3rem" }}>
        ...
        >
          ...
          >
              ...
              <Heart
                ...
                fill={
                  ...
                  project.likes.find(
                    (l) =>
                      l.user &&
                      (l.user._id || l.user) === localStorage.getItem("userId"),
                  )
                    ? "currentColor"
                    : "none"
                }
            ...
            >
              <MessageSquare size={24} /> {project.comments?.length || 0}{" "}
              Comments
            </div>
          </div>
              ...
              project.comments.map((comment, idx) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {comment.name || comment.user?.username || "Anonymous"}
                    </span>
                    <span
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              <button onClick={addOpenRole} className="btn-primary">
                Add Role
              </button>
            </div>
          </div>
