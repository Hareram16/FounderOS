...
try {
  ...
  console.warn(
    "Gemini AI library missing or initialization failed. Using Mock Fallback.",
    error.message,
  );
}
...
const generateSmartMock = (type, data) => {
  try {
    ...
    if (type === "validate") {
      ...
      if (keywords.includes("environment") || keywords.includes("eco"))
        strengths.push("Sustainability focus is a strong trend");
      if (keywords.includes("ai") || keywords.includes("smart"))
        strengths.push("Leverages emerging tech");
      if (keywords.includes("social") || keywords.includes("connect"))
        strengths.push("Strong community potential");
    ...
    if (type === "leanCanvas") {
      const conversationText = Array.isArray(data.conversation)
      ...
      let keyMetrics =
        "What metrics will you track? (e.g., user growth, revenue)";
      ...
      if (
        ...
        problem =
          "Students and professionals struggle to identify relevant skills and learning paths aligned with market demands";
        solution =
          "AI-driven personalized learning platform that analyzes skills, career goals, and industry trends";
        customerSegments =
          "Students, professionals seeking career advancement, and companies looking for skilled talent";
        keyMetrics =
          "User growth, course completion rates, skill verification, job placement rates";
        channels =
          "Online marketing, partnerships with educational institutions, LinkedIn, job boards";
        revenueStreams =
          "Subscription fees, course sales, premium features, corporate partnerships";
        costStructure =
          "Platform development, AI infrastructure, content creation, marketing";
        uniqueValue =
          "Real-time job market integration and personalized skill recommendations";
        unfairAdvantage =
          "Proprietary algorithm that continuously adapts to market trends";
      }

      if (text.includes("market") || text.includes("job")) {
        channels =
          "Online platforms, social media, partnerships with employers";
      }
    ...
    if (type === "pitchDeck") {
      ...
      if (
        ...
        problem =
          "Students and professionals struggle to identify relevant skills and learning paths aligned with market demands. Traditional education systems don't adapt quickly to industry needs, leaving skill gaps.";
        ...
        market =
          "The global online education market is valued at $350B+ and growing at 9% annually. With 70% of professionals seeking skill upgrades, there's massive demand for personalized learning solutions.";
        businessModel =
          "Freemium SaaS model: Free basic access, Premium subscriptions ($29/month), Enterprise partnerships, Course marketplace commissions (20-30%), Certification fees.";
        traction =
          "Currently in MVP phase with 500+ beta users, partnerships with 3 educational institutions, and integration with major job platforms.";
        team =
          "Experienced team of 8 with backgrounds in AI/ML, education technology, and business development.";
        ask =
          "Seeking $500K seed funding to scale platform, expand content library, and accelerate user acquisition.";
      } else if (
        description.includes("ai") ||
        description.includes("artificial intelligence")
      ) {
        problem =
          "Businesses struggle to leverage AI effectively due to complexity, high costs, and lack of expertise.";
        solution = `AI-powered platform that makes advanced AI accessible through intuitive interfaces and automated workflows.`;
        market =
          "The AI market is projected to reach $1.8T by 2030. Small and medium businesses represent a $200B+ opportunity.";
        businessModel =
          "SaaS subscriptions ($99-$999/month tiers), API usage fees, Enterprise contracts, White-label licensing.";
        traction =
          "Launched 6 months ago with 200+ active customers, $50K MRR, and 95% customer satisfaction.";
        team =
          "Team of 12 including AI researchers, software engineers, and sales professionals.";
        ask =
          "Raising $2M Series A to expand engineering team, enhance AI capabilities, and scale marketing.";
      } else {
        // Generic smart defaults
        problem =
          "Current solutions in the market are inefficient, expensive, or don't address core customer needs effectively.";
        solution = `${title} provides an innovative approach that solves these challenges through technology and user-centric design.`;
        market =
          "The target market represents a significant opportunity with growing demand and limited effective solutions.";
        businessModel =
          "Revenue streams include subscription fees, transaction commissions, premium features, and enterprise licensing.";
        traction =
          "Early traction includes initial user base, positive feedback, and key partnerships in development.";
        team =
          "Experienced founding team with relevant industry expertise and proven track record.";
        ask =
          "Seeking funding to accelerate product development, expand team, and scale operations.";
      }
  ...
  try {
    ...
    if (Array.isArray(conversation)) {
      conversationText = conversation
        .map((c) => (typeof c === "string" ? c : c.content || c.text || ""))
        .join("\n");
  ...
  try {
    ...
    try {
      ...
      if (!responseSent && !res.headersSent) {
        ...
        const mockData = getMockData(projectData);
        console.log(
          "Sending mock data after API error:",
    ...
    try {
      ...
      if (!responseSent && !res.headersSent) {
        ...
        const mockData = getMockData(projectData);
        console.log(
          "Sending mock data after JSON parse error:",
          Object.keys(mockData),
        );
        return res.json(mockData);
      }
    ...
    if (!responseSent && !res.headersSent) {
      try {
        ...
        console.log(
          "Sending error response with mock data:",
          Object.keys(mockData),
        );
        ...
        if (!res.headersSent) {
          try {
            ...
            console.log(
              "Sending fallback error response:",
      ...
      console.error(
        "Cannot send error response - headers already sent or response already sent",
      );
    }
  }
});
...
router.post("/chat", async (req, res) => {
  try {
    ...
    if (!res.headersSent) {
      res.json({
        reply:
          "I'm having trouble connecting right now, but keep pushing forward!",
